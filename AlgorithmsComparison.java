...
public class AlgorithmsComparison {
    public static void main(String[] args) {
        ...
        Song[] songsArray = getAllSongs("./song_plays.csv"); //ensure the song_plays.csv is in the same folder as this java file
        /** PUT THE CALL to sort or search here */
        /** The task is to sort or search through the array*/
        songsArray = selectionSort(songsArray);

        for (int i = 0; i < songsArray.length; i++) {
            System.out.println(songsArray[i].getViews());
        }
    }
    ...
    private static Song[] quickSort(Song[] songsArray) {
        ...
        // All the above conversions were not in vain, note that quick sort takes array as
        // parameter not array list.
        // The concatenation method combines all the passed parameters into a single array
    ...
    public static Song[] concatenate(Song[] leftArr, Song pivot, Song[] rightArr) {
        // We need the length of the left and right arrays to determine the length of the
        // final array
        ...
        // Now we can create the final songs array that is sufficient to hold all the songs.
        Song[] finalSongsArray = new Song[totalSize];

        // this i will be used to track the indices of th final songs array so it should have this 
        // outside scope
        int i = 0;
        for (; i < sizeLeft; i++) {
